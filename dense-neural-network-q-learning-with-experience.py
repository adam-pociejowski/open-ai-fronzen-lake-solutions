...
inputs = tf.placeholder(tf.float32, [None, 16])
W = tf.Variable(tf.random_uniform([16, 4], 0, 0.01))
Q = tf.matmul(inputs, W)
predict = tf.argmax(Q, 1)

Qnext = tf.placeholder(tf.float32, [None, 4])
...
def update_experience_batch(observation, targetQ):
    batch.observations.append(np.asarray(observation).reshape(16))
    batch.actions.append(np.asarray(targetQ).reshape(4))
    if len(batch.observations) > experience_batch_size:
...
with tf.Session() as sess:
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_episode_step):
            ...
            update_experience_batch(np.identity(16)[observation:observation + 1], targetQ)

            # Train network using target and predicted Q values
            # _, W1 = sess.run([updateModel, W], feed_dict={inputs: np.identity(16)[observation:observation + 1],
            #                                               Qnext: targetQ})

            _, W1 = sess.run([updateModel, W], feed_dict={inputs: batch.observations, Qnext: batch.actions})
